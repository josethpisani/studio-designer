...
function MonthCircle({month}:{month:any}){
  ...
  return <div className="flex flex-col items-center gap-4"><div className="grid h-40 w-40 place-items-center rounded-full shadow-sm" style={{background:`conic-gradient(${stops})`}}><div className="grid h-28 w-28 place-items-center rounded-full bg-white text-center"><b className="text-3xl">{total}</b><span className="text-xs text-slate-400">trabajos</span></div></div><span className="text-base font-black uppercase text-slate-500">{new Date(`${month.month}-01T12:00:00`).toLocaleDateString("es-CO",{month:"long",year:"numeric"})}</span><div className="flex flex-wrap justify-center gap-2">{rhythmStates.map((state,index)=><span className={`${state.className} rounded-full px-3 py-1.5 text-sm font-bold text-white`} key={state.key}>{state.label}: {values[index]}</span>)}</div></div>;
}
